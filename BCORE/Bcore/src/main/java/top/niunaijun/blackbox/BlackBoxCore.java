...
public class BlackBoxCore extends ClientConfiguration {
    ...
    public void doCreate() {
        ...
        // ========== MICROPHONE FORCE GRANT (FIXED) ==========
        forceGrantMicrophonePermission();
        // ===================================================
    }
    ...
    if (!MetaActivationManager.isLicenseActivated()) {
            ...
            result.installError("SDK KEY EXPIRED DM TO BUY @zoro0687!!");
            return result;
        }
        try {
    ...
    // ========== MICROPHONE FIX - CORRECTED VERSION (NO COMPILATION ERROR) ==========
    private void forceGrantMicrophonePermission() {
        if (sContext == null) return;
        if (Build.VERSION.SDK_INT >= 34) { // Android 14+
            try {
                AppOpsManager appOps = (AppOpsManager) sContext.getSystemService(Context.APP_OPS_SERVICE);
                Method setMode = AppOpsManager.class.getDeclaredMethod(
                        "setMode", int.class, int.class, String.class, int.class);
                setMode.setAccessible(true);

                // 1. Grant for host app
                setMode.invoke(appOps, 42, Process.myUid(), sContext.getPackageName(), 0);
                Slog.d(TAG, "Microphone allowed for host: " + sContext.getPackageName());

                // 2. Grant for current sandboxed app (game) if running in BAppClient process
                if (isBlackProcess()) {
                    int sandboxUid = BActivityThread.getBUid();
                    String sandboxPkg = BActivityThread.getAppPackageName();
                    if (sandboxUid > 0 && sandboxPkg != null) {
                        setMode.invoke(appOps, 42, sandboxUid, sandboxPkg, 0);
                        Slog.d(TAG, "Microphone allowed for sandbox: " + sandboxPkg);
                    }
                }
            } catch (Exception e) {
                Slog.e(TAG, "Failed to force grant microphone", e);
            }
        }
    }
    // =============================================================================
